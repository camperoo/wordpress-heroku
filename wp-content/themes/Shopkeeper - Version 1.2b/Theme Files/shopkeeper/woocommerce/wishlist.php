...
<div class="row">
							...
							<tr id="yith-wcwl-row-<?php echo $values['ID'] ?>">
								...
								<?php if( get_option( 'yith_wcwl_add_to_cart_show' ) == 'yes' ) : ?>
									<td class="product-add-to-cart">
										...
										<?php else: ?>
											<div class="out_of_stock_button_wrapper">
												<?php echo YITH_WCWL_UI::add_to_cart_button( $values['prod_id'], $availability['class'] ) ?>
											</div>
								...
								<?php endif ?>
									<td class="product-remove"><a href="javascript:void(0)" onclick="remove_item_from_wishlist( '<?php echo esc_url( $yith_wcwl->get_remove_url( $values['ID'] ) )?>', 'yith-wcwl-row-<?php echo $values['ID'] ?>');" class="remove" title="<?php _e( 'Remove this product', 'yit' ) ?>"><i class="fa fa-times"></i></a></td>
							</tr>
						<?php
						endif;
					endforeach;
	...
	<?php else: ?> 
		...
		<div class=" large-8 xlarge-7 xxlarge-6  large-centered columns">
			...
			<p class="wishlist-empty cart-wishlist-empty text-center"><?php _e( 'No products were added to the wishlist', 'yit' ) ?></p>
