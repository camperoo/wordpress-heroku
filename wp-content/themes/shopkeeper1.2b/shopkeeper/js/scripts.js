...
jQuery(document).ready(function ($) {
	...
	//search
	$('#searchform #s, .search-form .search-field').val('');
	
	$(".search-button").on('click',function() {
		
		var documentHeight = $(document).height(),
			searchWrapper = $('.site-search .search-form'),
			searchProductsWrapper = $('.site-search #searchform > div');
		
		//woocommerce search
		if ( $('.site-search #searchform').length > 0 ) {
		
			$('.site-search #searchform #s').val('');
			...
			$('.site-search #searchform #s').focus();
	...
	//load images 
	var images = $('.parallax, .single-post-header-bkg');
