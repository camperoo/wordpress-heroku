...
function refresh_dynamic_contents() {
	...
	$data = array(
		'cart_count_products' => $woocommerce->cart->cart_contents_count,
...
add_filter( 'wp_title', 'shopkeeper_wp_title', 10, 2 );
function shopkeeper_wp_title( $title, $sep ) {
	global $paged, $page;
	...
	if ( $paged >= 2 || $page >= 2 ) {
		$title = "$title $sep " . sprintf( __( 'Page %s', 'shopkeeper' ), max( $paged, $page ) );
	}
