...
if ( ! function_exists( 'rey_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function rey_posted_on() {
	...
	$posted_on = sprintf(
		_x( 'Posted on %s', 'post date', 'rey' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		_x( 'by %s', 'post author', 'rey' ),
 ...
 * @return bool
 */
function rey_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'rey_categories' ) ) ) {
		...
		set_transient( 'rey_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so rey_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so rey_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in rey_categorized_blog.
 */
function rey_category_transient_flusher() {
	// Like, beat it. Dig?
	delete_transient( 'rey_categories' );
}
add_action( 'edit_category', 'rey_category_transient_flusher' );
add_action( 'save_post',     'rey_category_transient_flusher' );
